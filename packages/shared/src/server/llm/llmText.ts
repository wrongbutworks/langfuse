...
import type { TranslatedProviderOptions } from "./ai-sdk/providers/types";
...
export type LLMCredentialSource = "connection" | "langfuse-managed";
...
>(
  ...
  const credentialSource = options.credentialSource ?? "connection";
  const timeout =
  ...
  try {
    ...
    const languageModel = await buildAiSdkModel({
      ...
      useManagedCredentials: credentialSource === "langfuse-managed",
      createFetch,
    });
    ...
    return {
      ...
      callOptions: {
        ...
        // AI SDK otherwise downloads media URLs that the selected model cannot
        // consume directly using global fetch, bypassing Langfuse's outbound
        // URL validation. Provider-supported URLs remain pass-through; callers
        // can use inline data for every other media input.
        experimental_download: rejectUnsupportedMediaUrlDownloads,
...
const rejectUnsupportedMediaUrlDownloads: Experimental_DownloadFunction =
  async (downloads) => {
    if (downloads.some(({ isUrlSupportedByModel }) => !isUrlSupportedByModel)) {
      throw new LLMCompletionError({
        message:
          "Remote media URLs are not supported by this model; use inline data instead",
...
export function mapLegacyLLMCompletionParams(params: {
  ...
  return {
    ...
    providerOptions,
    credentialSource: params.credentialSource ?? "connection",
  };
}
