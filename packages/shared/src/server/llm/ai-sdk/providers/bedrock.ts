...
export function buildBedrockModel(params: {
  ...
  useManagedCredentials: boolean;
}): LanguageModel {
  const { modelId, apiKey, config, useManagedCredentials } = params;

  const { region } = useManagedCredentials
  ...
  const auth = resolveBedrockProviderAuth({
    secretKey: apiKey,
    allowDefaultCredentials: isSelfHosted || useManagedCredentials,
  });
