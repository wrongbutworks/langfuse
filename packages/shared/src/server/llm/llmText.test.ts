...
describe("generateLLMText", () => {
  ...
  it("does not download media URLs that the model cannot consume directly", async () => {
  ...
  it("passes model-supported media URLs through without downloading them", async () => {
    const model = new MockLanguageModelV4({
    ...
    expect(result.text).toBe("image accepted");
    expect(fetchSpy).not.toHaveBeenCalled();
...
describe("legacy compatibility boundary", () => {
  ...
  it("rejects invalid connections and unsupported managed credentials", async () => {
    ...
    await expect(
      generateLLMText({
        ...openAIOptions(),
        credentialSource: "langfuse-managed",
      }),
    ).rejects.toMatchObject({
      name: "LLMCompletionError",
      message:
        "Langfuse-managed credentials are only supported for Amazon Bedrock",
      responseStatusCode: 400,
      isRetryable: false,
    });
  });
